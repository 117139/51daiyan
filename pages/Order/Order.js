...
Page({
  ...
  select() {
    var that = this
    that.setData({
      my_dou_xuan: !that.data.my_dou_xuan
    });
    that.jisuan()
  },
  jisuan() {
    var that = this
    var yhnum = that.data.yhmsg ? that.data.yhmsg.arg1 : ''
    if (that.data.my_dou_xuan) {
      var zsum = that.data.sum - yhnum
      var my_dou_new = that.data.my_dou
      if (zsum > that.data.my_dou) {
        zsum = zsum - that.data.my_dou
        my_dou_new = 0
      } else {
        my_dou_new = that.data.my_dou - zsum
        zsum = 0
      }
      console.log(zsum)
